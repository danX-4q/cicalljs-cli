...
const myToken = qtum.contract("cicall-solidity/ContractA.sol")
const myTokenB = qtum.contract("cicall-solidity/ContractB.sol")
const myTokenC = qtum.contract("cicall-solidity/ContractC.sol")
...
async function transferCCC(fromAddr,  toAddr1, toAddr2, toAddr3, walletAddr, amount, value1,  values2, refundValue) {
  const tx = await myToken.send("transferCCC", [toAddr2, toAddr3, walletAddr, value1,  values2, refundValue], {
    senderAddress: fromAddr,
    amount: amount
  })

  console.log("transferCCC tx:", tx.txid)

  // or: await tx.confirm(1)
  const confirmation = tx.confirm(1)
  ora.promise(confirmation, "confirm transferCCC")
  await confirmation
}
...
async function main() {
  ...
  switch (cmd) {
    ...
    {
      const fromAddr = argv._[1]
      const toAddr1 = argv._[2]
      const toAddr2 = argv._[3]
      const toAddr3 = argv._[4]
      const walletAddr = argv._[5]
      var amount = argv._[6]    //unit is always eth!
      var value1 = argv._[7]
      var value2 = argv._[8]
      var refundValue = argv._[9]
      const unit = argv._[10]
      ...
      //console.log(amount)
      //return
      await transferCCC(fromAddr, toAddr1, toAddr2,  toAddr3, walletAddr, amount, value1,  value2, refundValue)
      break
    }
    case "getbalance":
    {
